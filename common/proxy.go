...
func Proxy(conn net.Conn, ws *websocket.Conn) {
	...
	go func() {
		...
		conn.SetReadDeadline(time.Now().Add(ProxyIdleTimeout))
		// Set write deadline for the WebSocket connection
		ws.SetWriteDeadline(time.Now().Add(ProxyIdleTimeout))
	...
	go func() {
		...
		ws.SetReadDeadline(time.Now().Add(ProxyIdleTimeout))
		// Set write deadline for the network connection
		conn.SetWriteDeadline(time.Now().Add(ProxyIdleTimeout))
